...
import {
  Globe, Settings, UserCheck,
  Users, Package, FileText, Calendar, Wrench, PenLine, BarChart2, MessageSquare, Bell,
  Inbox, Wifi, Receipt,
  MapPin, Building2, Car, Leaf, Briefcase, HeartPulse,
  type LucideIcon,
} from "lucide-react";

/* Icon key → component map */
...
function Icon({ name, className }: { name: string; className?: string }) {
  const I = ICONS[name];
  return I ? <I className={className ?? "h-5 w-5"} /> : null;
}
...
const content = {
  fr: {
    ...
    pillars: [
      {
        ...
        desc: "Votre image professionnelle en ligne. Présentation de vos services, formulaires de contact, génération de leads — votre meilleur commercial, disponible en continu.",
    ...
    features: [
      { icon: "Users",          title: "Gestion des équipes",      desc: "Fiches employés, rôles, droits d'accès, congés — pilotez vos ressources humaines sans Excel." },
      { icon: "Package",        title: "Stocks & inventaires",     desc: "Suivi en temps réel des stocks, alertes de réapprovisionnement, historique des mouvements." },
      { icon: "FileText",       title: "Devis & facturation",      desc: "Créez, envoyez et suivez vos devis et factures en quelques clics. Relances automatiques." },
      { icon: "Calendar",       title: "Planning & agenda",        desc: "Planifiez les interventions, affectez les équipes et synchronisez les agendas automatiquement." },
      { icon: "Wrench",         title: "Suivi des interventions",  desc: "Chaque intervention est tracée de l'ouverture à la clôture. Rapports terrain en temps réel." },
      { icon: "PenLine",        title: "Signature électronique",   desc: "Vos clients signent leurs devis en ligne, depuis n'importe quel appareil. Validité juridique." },
      { icon: "BarChart2",      title: "Tableau de bord",          desc: "Vue consolidée de votre activité : CA, interventions, équipes, satisfaction client." },
      { icon: "MessageSquare",  title: "Demandes clients",         desc: "Centralisez toutes les demandes entrantes. Triez, assignez et traitez sans rien manquer." },
      { icon: "Bell",           title: "Notifications & alertes",  desc: "Vos équipes et vos clients reçoivent les bonnes informations au bon moment, automatiquement." },
    ],
    ...
    steps: [
      { num: "01", icon: "Inbox",       label: "Demande client",  desc: "Le client soumet une demande via le site vitrine ou le portail client." },
      { num: "02", icon: "FileText",    label: "Devis",           desc: "Votre équipe crée et envoie un devis personnalisé en quelques minutes." },
      { num: "03", icon: "PenLine",     label: "Signature",       desc: "Le client signe le devis électroniquement depuis son espace client." },
      { num: "04", icon: "Calendar",    label: "Planification",   desc: "L'intervention est planifiée et les équipes sont automatiquement notifiées." },
      { num: "05", icon: "Wrench",      label: "Intervention",    desc: "Le technicien suit et met à jour l'avancement depuis son mobile." },
      { num: "06", icon: "Wifi",        label: "Suivi client",    desc: "Le client voit l'avancement en temps réel dans son portail." },
      { num: "07", icon: "Receipt",     label: "Facture",         desc: "La facture est générée et envoyée automatiquement à la clôture." },
    ],
    ...
    portalFeatures: [
      { icon: "MapPin",        title: "Suivi en temps réel",    desc: "Avancement de chaque demande ou intervention mis à jour automatiquement." },
      { icon: "FileText",      title: "Documents centralisés",  desc: "Devis, factures, rapports d'intervention accessibles à tout moment." },
      { icon: "PenLine",       title: "Signature en ligne",     desc: "Validation et signature de devis sans impression ni déplacement." },
      { icon: "MessageSquare", title: "Messagerie directe",     desc: "Échanges avec votre équipe sans passer par email." },
    ],
    ...
    gains: [
      ...
      { label: "Appels de suivi client",                  before: "40/semaine", after: "5/semaine",    delta: "-87%" },
      { label: "Erreurs de planification",                before: "Fréquentes", after: "Quasi nulles", delta: "-95%" },
    ],
    sectorsTitle: "Conçu pour les PME de services",
    sectors: [
      { icon: "Wrench",      label: "Plomberie, électricité, CVC" },
      { icon: "Building2",   label: "BTP et rénovation" },
      { icon: "Car",         label: "Maintenance et dépannage" },
      { icon: "Leaf",        label: "Paysagisme, nettoyage" },
      { icon: "Briefcase",   label: "Cabinets et prestataires" },
      { icon: "HeartPulse",  label: "Santé et paramédical" },
    ],
  ...
  en: {
    ...
    pillars: [
      {
        ...
        desc: "Your professional image online. Service presentation, contact forms, lead generation — your best salesperson, available continuously.",
    ...
    features: [
      { icon: "Users",          title: "Team management",          desc: "Employee profiles, roles, access rights, leave — manage your human resources without spreadsheets." },
      { icon: "Package",        title: "Stock & inventory",        desc: "Real-time stock tracking, restocking alerts, movement history." },
      { icon: "FileText",       title: "Quotes & invoicing",       desc: "Create, send and track quotes and invoices in a few clicks. Automatic reminders." },
      { icon: "Calendar",       title: "Planning & scheduling",    desc: "Schedule interventions, assign teams and sync calendars automatically." },
      { icon: "Wrench",         title: "Intervention tracking",    desc: "Every intervention is tracked from opening to closing. Real-time field reports." },
      { icon: "PenLine",        title: "Electronic signature",     desc: "Clients sign quotes online, from any device. Legally valid." },
      { icon: "BarChart2",      title: "Dashboard",                desc: "Consolidated view of your activity: revenue, interventions, teams, client satisfaction." },
      { icon: "MessageSquare",  title: "Customer requests",        desc: "Centralise all incoming requests. Sort, assign and process without missing anything." },
      { icon: "Bell",           title: "Notifications & alerts",   desc: "Your teams and clients receive the right information at the right time, automatically." },
    ],
    ...
    steps: [
      { num: "01", icon: "Inbox",       label: "Customer request",  desc: "The customer submits a request via the showcase site or client portal." },
      { num: "02", icon: "FileText",    label: "Quote",             desc: "Your team creates and sends a personalised quote in minutes." },
      { num: "03", icon: "PenLine",     label: "Signature",         desc: "The customer signs the quote electronically from their client space." },
      { num: "04", icon: "Calendar",    label: "Scheduling",        desc: "The intervention is scheduled and teams are automatically notified." },
      { num: "05", icon: "Wrench",      label: "Intervention",      desc: "The technician tracks and updates progress from their mobile." },
      { num: "06", icon: "Wifi",        label: "Client tracking",   desc: "The customer sees real-time progress in their portal." },
      { num: "07", icon: "Receipt",     label: "Invoice",           desc: "The invoice is automatically generated and sent on completion." },
    ],
    ...
    portalFeatures: [
      { icon: "MapPin",        title: "Real-time tracking",       desc: "Progress of every request or intervention updated automatically." },
      { icon: "FileText",      title: "Centralised documents",    desc: "Quotes, invoices, intervention reports accessible at any time." },
      { icon: "PenLine",       title: "Online signature",         desc: "Quote validation and signing without printing or travel." },
      { icon: "MessageSquare", title: "Direct messaging",         desc: "Exchanges with your team without going through email." },
    ],
    ...
    gains: [
      ...
      { label: "Client follow-up calls",          before: "40/week",    after: "5/week",        delta: "-87%" },
      { label: "Scheduling errors",               before: "Frequent",   after: "Near zero",     delta: "-95%" },
    ],
    sectorsTitle: "Built for service SMEs",
    sectors: [
      { icon: "Wrench",      label: "Plumbing, electrical, HVAC" },
      { icon: "Building2",   label: "Construction and renovation" },
      { icon: "Car",         label: "Maintenance and repairs" },
      { icon: "Leaf",        label: "Landscaping, cleaning" },
      { icon: "Briefcase",   label: "Consultancies and service providers" },
      { icon: "HeartPulse",  label: "Healthcare and paramedical" },
    ],
            ...
            <div className="grid grid-cols-2 gap-px bg-white/5 lg:grid-cols-4">
              {[
                ...
                <div key={l as string} className="bg-[#040b18] p-6 text-center">
                  <div className="text-3xl font-black" style={{ color: clr as string }}>{v}</div>
            ...
            {c.pillars.map((p) => (
              <div key={p.num} className="group relative overflow-hidden border border-slate-200 bg-white p-8 transition-all duration-300 hover:border-slate-300 hover:shadow-lg">
                <div className="mb-6 flex items-start justify-between">
                  <div className="flex h-12 w-12 items-center justify-center" style={{ background: `${p.color}12`, color: p.color }}>
                ...
                {c.steps.map((step, i) => {
                  ...
                  return (
                    <div key={step.num} className="relative flex flex-col items-center text-center">
                      <div className="relative z-10 flex h-16 w-16 items-center justify-center border border-white/10" style={{ background: `${colors[i]}20`, color: colors[i] }}>
            ...
            {c.steps.map((step, i) => {
              ...
              return (
                <div key={step.num} className="flex gap-4 border border-white/10 bg-white/5 p-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center" style={{ background: `${colors[i]}20`, color: colors[i] }}>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10">
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <div>
              <div className="mb-5 inline-block bg-cyan-600 px-3 py-1 text-[11px] font-black uppercase tracking-widest text-white">{c.portalBadge}</div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={label as string} className="flex items-center justify-between border border-slate-100 px-4 py-3">
                      ...
                      <span className="text-[11px] font-bold" style={{ color: clr as string }}>{status}</span>
                    </div>
                  ))}
                </div>
              </div>
